...
5. ~~**Existing VNet private endpoint testing should report only what's missing.**~~ **DONE.** On the existing-VNet + existing-zones path, per-zone present+linked rows are suppressed; only missing (Fail) and unlinked (Warn) zones emit rows, plus a rollup — a single "all present and linked" Pass when clean, or an Info summary ("N of M ... linked") whose Message carries the full per-zone state for the JSON output.
